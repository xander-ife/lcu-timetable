    ...
    <div style={{padding:isMobile?14:22}}>
      <Card style={{marginBottom:14}}>
        ...
        <div style={{padding:"4px 14px 14px"}}>
          ...
          <InfoRow label="Matriculation Number" value={user.id}/>
...
const SettingsPage = ({isMobile}) => (
  <div style={{padding:isMobile?14:22}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {[["🎓 Academic Session","Configure current session and academic calendar."],["🔐 Security","JWT expiry, session timeout (15 min), password policy."],["📧 Notifications","Email alerts for timetable changes and conflicts."],["💾 Database","Backup, indexing, ACID transactions, query optimization."],["🖨 PDF Templates","Customize timetable export with LCU logo and layout."],["👥 User Management","Add/remove users, reset passwords, manage roles."]].map(([title,desc])=>(
        <Card key={title}>
          <div style={{padding:"14px"}}>
            ...
            <button style={{padding:"7px 14px",background:"#fff",border:`1px solid ${C.navy}`,borderRadius:4,color:C.navy,fontWeight:600,fontSize:13,cursor:"pointer"}}>Configure →</button>
          </div>
        </Card>
      ))}
    </div>
